...
## 制作背景(意図)
  絵を書いたりグッズやアクセサリーを作る器用な芸人さんが増えている。
  しかし、それらを購入するには個々の専用HPにアクセスしなければならず、アクセス回数が増えて不便。
  多数の芸人さんの作品をひとまとめに閲覧・購入することができれば、
  ①個々のHPへアクセスという手間の削減
  ②作品をきっかけにさまざまな芸人さんを知る機会増加
  ③作品をきっかけにファンを作る機会増加
  といった不便を解消するだけでなく、芸人さんと一般ユーザー双方にとってもメリットが見込める。
...
| ストーリー | 新規登録ボタンを押すと新規登録ページに遷移する。新規登録が完了すると、ログインした状態でトップページへ遷移する。 未ログイン状態でログインボタンを押すとログインページに遷移する。ログインボタンを押すとトップページへ遷移する。|
| 見積もり | 5時間 |
...
| ストーリー | 商品一覧ページに出品ボタンがあり、クリックすると出品ページへ遷移する。出品完了するとトップページへ戻り、商品一覧ページに出品商品が追加される。|
| 見積もり | 5時間 |
...
| 詳細 | 出品者が商品出品時に登録した内容が表示され、一覧ページに表示されている内容より詳細な情報を確認することができる。
戻るボタンが共通で表示されるがその他のボタンとして、
一般ユーザーの場合は購入ボタンを表示、出品者の場合は編集・削除ボタンを表示、未ログインユーザーの場合はボタンの表示はない。|
| ストーリー | 商品をクリックすると詳細ページに遷移し、商品詳細を確認することができる。
購入・編集・削除ボタンをクリックすると、それぞれのページへ遷移する。戻るボタンをクリックすると、一つ前のページへ遷移する。|
| 見積もり | 3時間 |
...
| ストーリー | ログインユーザーが出品者の時、商品詳細ページに編集ボタンが表示される。編集ボタンをクリックすると、編集ページへ遷移し内容を編集することができる。更新ボタンをクリックすると、商品詳細ページへ遷移し、更新内容が表示される。|
| 見積もり | 2時間 |
...
| ストーリー | ログインユーザーが出品者の時、商品詳細ページに削除ボタンが表示される。削除ボタンをクリックすると削除が完了してユーザー詳細ページへ遷移する。|
| 見積もり | 2時間 |
...
| 詳細 | ログインユーザーが一般ユーザーの時のみ、商品詳細ページに購入ボタンを表示する。
購入確認ページでクレジットカードや配送先の情報を入力すると購入することができる。|
| ストーリー | 商品詳細ページの購入ボタンをクリックすると、購入確認ページへ遷移する。必要な情報を入力して購入ボタンをクリックすると、購入完了しトップページへ遷移する。
購入したユーザーのユーザー詳細ページには購入した商品が表示される。|
| 見積もり | 8時間 |
...
| 詳細 | ユーザーのプロフィール情報を表示。ユーザーが出品者の場合は出品商品が一覧表示される。
一般ユーザーの場合はいいねした商品や購入した商品が表示される。|
| ストーリー | ログイン中のユーザー名や商品詳細のユーザー名をクリックするとユーザー詳細ページへ遷移する。表示されている商品をクリックすると商品詳細ページへ遷移できる|
| 見積もり | 4時間 |
...
| ストーリー | 商品一覧ページのヘッダーにある芸人名・所属事務所・商品カテゴリ・金額範囲のどれか一つでも指定して検索ボタンをクリックすると、検索結果ページに遷移して、該当する商品が表示される。|
| 見積もり | 8時間 |
...
| 詳細 | いいねした商品はユーザー詳細ページに一覧で表示される。
出品者本人や未ログインユーザーはいいねできず、いいね数のみ見ることができる。|
| ストーリー | 商品一覧ページで♡をクリックするといいねしたサインとして色が変わり、横の数字が1つカウントされる。いいねされた状態で再度クリックすると、いいねは外されカウントも1つ下がる。一般ユーザーはユーザー詳細ページへ移動すると、自分がいいねした商品を見ることができる。|
| 見積もり | 8時間 |
...
### 工夫したポイント
  1. 利用方法別に、ユーザー新規登録時の内容を変更
    - ①芸人として利用するのであれば、所属事務所・芸歴・自己紹介
    - ②一般ユーザーとして利用するのであれば、自己紹介のみの入力と、ユーザーによって登録内容を変更できるようにした。
  
  2. ログインユーザーによって各ページの表示内容を変更。
    1. 芸人として利用する場合
    - 商品一覧ページに出品ボタンを表示した。
      ![talent-cell](https://user-images.githubusercontent.com/77190650/111932958-d897fe80-8b01-11eb-8fc6-19b486c01e84.png)
    - ユーザー詳細ページには、出品した作品一覧を表示した。
    - ユーザー詳細ページのサイドプロフィールには所属事務所・芸歴・自己紹介文を表示した。
    - 芸人さんは作品出品のみの利用のため、他の芸人の商品は購入できないように、商品詳細ページの購入ボタンを非表示にした。
    - 自分が出品した作品のみ編集・削除できるようにした。
    - 自分が出品した作品にはいいねクリックできないよう表示変更した。

    2. 一般ユーザーとして利用する場合
    - 一般ユーザーは作品購入のみの利用のため、商品一覧ページの出品ボタンを非表示にした。
      ![ippan-nosell](https://user-images.githubusercontent.com/77190650/111933185-49d7b180-8b02-11eb-9ab6-e15d7d2a1f0c.png)
    - ユーザー詳細ページに、いいね押した作品一覧を表示した。
    - ユーザー詳細ページのサイドプロフィールには自己紹介文と過去の購入済作品名を表示した。
      ![ippan-page](https://user-images.githubusercontent.com/77190650/111952396-71da0b80-8b28-11eb-89ec-4199189b8d59.png)

    3. 未ログインで利用する場合
    - 未ログインユーザーは作品購入できないように、商品詳細ページの購入ボタンを非表示にした。
    - 未ログインユーザーはいいねクリックできないように表示変更した。
      ![un-login](https://user-images.githubusercontent.com/77190650/111953058-6d622280-8b29-11eb-9be4-2c789f8e9d4c.png)

  3. 添付画像プレビュー機能
    - 商品出品時に添付する画像をプレビュー表示させて、画像確認をできるようにした。
      ![preview](https://user-images.githubusercontent.com/77190650/111934333-8b695c00-8b04-11eb-9227-c74a269443ab.png)

  4. いいね機能追加
    - 気に入った作品はいいねをすることができ、一般ユーザーであればいいねした作品をユーザー詳細ページでみることができる。
    - 芸人さんは、ユーザー詳細ページで出品商品についたいいね数を確認することができる。
    - 非同期通信を使い、ページ遷移せずにいいねの付け外しをできるようにした。

  5. カテゴリー検索機能追加
    - 芸人さんの名前・所属事務所・カテゴリー・金額の範囲をそれぞれ指定して検索できるようにした。

  6. レスポンシブデザイン
    - スマホでも手軽に利用してもらうために、スマホのサイズにレスポンシブ化した。
    - また利用する時のことを考え、出品ボタンや購入ボタンを画面の右側に配置した。
